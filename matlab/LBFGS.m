...
while true
    ...
    if iter == 1
         d = - B * g;
         fprintf('The vector d is: [');
         fprintf('%g ', d);
         fprintf(']\n');
    else
        d = r1;
        fprintf('The vector d becomes: [');
        fprintf('%g ', d);
        fprintf(']\n');
    end
    ...
    saved_s = [saved_s(:,2:end),s];
    saved_y = [saved_y(:,2:end),y];
    rho = y' * s;
    if rho < 1e-16
       fprintf( '\nError: y^i s^i = %1.2e\n' , rho ); 
       status = 'error';
    ...
    for i = 0:bound-1
        alpha(M-i) = rhos(M-i)*dot(saved_s(:,M-i),q);
        q = q - alpha(M-i) * saved_y(:,M-i);
    end

    r1 = - B * q;

    for i = 1:bound
        beta = rhos(i) * dot(saved_y(:,i), r1);
        r1 = r1 + (alpha(i) - beta) * saved_s(:,i);
    end
